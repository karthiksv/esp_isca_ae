...
module vitdodec_plm_block_out_dma32(
  ...
  reg               bank_CE  [0:0][1:0][4:0][0:0][1:0];
  reg        [10:0] bank_A   [0:0][1:0][4:0][0:0][1:0];
  reg         [7:0] bank_D   [0:0][1:0][4:0][0:0][1:0];
  reg               bank_WE  [0:0][1:0][4:0][0:0][1:0];
  reg         [7:0] bank_WEM [0:0][1:0][4:0][0:0][1:0];
  wire        [7:0] bank_Q   [0:0][1:0][4:0][0:0][1:0];
  wire        [0:0] ctrld    [4:1];
  wire        [0:0] ctrlh    [4:0];
  wire        [2:0] ctrlv    [4:0];
  reg         [0:0] seld     [4:1];
...
// synopsys translate_off
  integer check_bank_access [0:0][1:0][4:0][0:0][1:0];
...
// synthesis translate_on
  ...
  for (h = 0; h < 2; h = h + 1) begin : gen_ctrl_hbanks
    for (v = 0; v < 5; v = v + 1) begin : gen_ctrl_vbanks
...
// synthesis translate_on
  ...
  for (hh = 0; hh < 1; hh = hh + 1) begin : gen_q_assign_hhbanks
    ...
    if (hh == 0 && (hh + 1) * 8 > 8) begin : gen_q_assign_hhbanks_last_3 
       assign Q3[7:8 * hh] = bank_Q[seld[3]][selh[3]][selv[3]][hh][1][7:0];
    end else begin : gen_q_assign_hhbanks_others_3 
      assign Q3[8 * (hh + 1) - 1:8 * hh] = bank_Q[seld[3]][selh[3]][selv[3]][hh][1];
    end
  ...
  for (d = 0; d < 1; d = d + 1) begin : gen_wires_dbanks
    for (h = 0; h < 2; h = h + 1) begin : gen_wires_hbanks
      for (v = 0; v < 5; v = v + 1) begin : gen_wires_vbanks
